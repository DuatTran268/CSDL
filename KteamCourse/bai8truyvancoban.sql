...
-- 2/ Tạo các table + Khoá chính
	...
	(
		...
		PHUCAP float,
		KETQUA nvarchar(10),
		Primary Key (MAGV,MADT,STT)	-- khóa chính bao gồm MAGV, MADT, STT
	)
	go
...
-- cấu trúc truy vấn
-- SELECT<*>: Lấy hết tất cả
select * from dbo.BOMON	-- lấy hết các dữ liệu trong bảng bộ môn ra
-- lấy mã đề tài và tên đề tài trong bảng bộ môn 
select MABM, TENBM from dbo.BOMON 
-- đổi tên cột hiển thị
select MABM as 'Cột 1', TENBM as N'Cột 2' from dbo.BOMON

-- lấy bảng giáo viên
select * from dbo.GIAOVIEN
select * from dbo.BOMON
-- xuất ra mã giáo viên + tên + bộ môn giáo viên đó dạy
select * from dbo.GIAOVIEN, dbo.BOMON		-- gộp 2 bảng
...
--2. lấy ra mã khoa và khoa tương ứng
